...
Money::Money(): pounds_(0), pence_(0) {}

Money::Money(const long pounds): pounds_(pounds), pence_(0) {}

Money::Money(const long pounds, const int pence): pounds_(pounds), pence_(pence) {}

Money Money::operator+(const Money& moneyRhs) const {
	// Convert all money to pence then do addition
	const long poundsInPence = (pounds_ + moneyRhs.pounds_) * 100;
	const int totalPence = pence_ + moneyRhs.pence_;
