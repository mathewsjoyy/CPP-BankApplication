...
void AccountManager::openAccount(const std::string& accountType, std::string accountBalance) {
	...
	const std::pair<bool, bool> canMakeCurrentIsa = doesCurrentISAExist(openedAccounts);  // Check if user already has current/ISA

	if (canComputeMoneyInput(pounds, pence, std::move(accountBalance)) == false) {  // Check money values can be found
		std::cout << ">- Unrecognizable opening balance value." << std::endl;
	}
	else {
		if (pounds < 0 || pence < 0) {
			...
			openedAccounts.push_back(std::make_unique<Savings>(pounds, pence));
			recentAccountIndex = openedAccounts.size() - 1; // set the most recently opened account
		}
		else if (accountType.compare("1") == 0) { // Create a current account
			if (canMakeCurrentIsa.first == false) {
				openedAccounts.push_back(std::make_unique<Current>(pounds, pence));
				recentAccountIndex = openedAccounts.size() - 1;
			}
			else {
			...
			if (canMakeCurrentIsa.second == false) {
				if (pounds < 1000) {
					...
					openedAccounts.push_back(std::make_unique<Savings>(pounds, pence, true));
					recentAccountIndex = openedAccounts.size() - 1;
				}
			}
			else {
...
void AccountManager::viewAccountsIndex(const std::string& index) {
	if (openedAccounts.empty()) { // Check if any account have been opened
		...
		try {
			...
			std::cout << *openedAccounts.at(intIndex);
			openedAccounts.at(intIndex)->getHistory();

			recentAccountIndex = intIndex; // Update recent account index
		}
		catch (std::exception&) {
...
void AccountManager::withdraw(std::string amount) const {
	...
	if (canComputeMoneyInput(pounds, pence, std::move(amount)) == false) {
		...
		openedAccounts.at(recentAccountIndex)->withdraw(pounds, pence); // Withdraw from account
	}
}
...
void AccountManager::deposit(std::string amount) const {
	...
	if (canComputeMoneyInput(pounds, pence, std::move(amount)) == false) {
		...
		openedAccounts.at(recentAccountIndex)->deposit(pounds, pence); // Deposit into the account
	}
}
...
void AccountManager::transfer(const std::string& takeMoneyAcc, const std::string& giveMoneyAcc, std::string amount) const {
	...
	try {
		...
		const int openedAccountsSize = static_cast<int>(openedAccounts.size()) - 1;
	...
	if (canComputeMoneyInput(pounds, pence, std::move(amount)) == false) {
		...
		// Check withdrawal is successful first
		if (openedAccounts.at(accountGiving)->withdraw(pounds, pence, withdrawMessage)) {
			openedAccounts.at(accountReceiving)->deposit(pounds, pence, depositMessage);
			std::cout << ">- All transactions successful. Transfer complete." << std::endl;
		}
		else { // Unsuccessful withdrawal, then no transactions are made
			std::cout << ">- No transactions made. Try again later." << std::endl;
		}
	}
}
...
void AccountManager::project(const std::string& years) const {
	...
	else
	{
		try {
			...
			const Savings& chosenSavingsAcc = dynamic_cast<Savings&>(*openedAccounts.at(recentAccountIndex));
...
void AccountManager::search(std::string value) const {
	...
	if (canComputeMoneyInput(pounds, pence, std::move(value)) == false) {  // Check money values can be found
		...
		openedAccounts.at(recentAccountIndex)->searchTransactionsByAmount(Money(pounds, pence));
	}
}
