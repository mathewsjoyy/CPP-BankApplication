...
void AccountManager::project(const std::string& years) const {
	...
	if (opened_accounts_.empty()) {
		...
		try {
			...
			fmt.copyfmt(std::cout);   // and copies the stream's format state into the variable

			// Call compute interest function to find projected value on the account and round output too 2 decimal places
			std::cout << ">- Projected Balance: \x9C" << std::setprecision(4) << chosenSavingsAcc.computeInterest(intYears) << std::endl;
