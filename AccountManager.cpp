...
void AccountManager::openAccount(const std::string & accountType, std::string accountBalance) {
	...
	if (canComputeMoneyInput(pounds, pence, std::move(accountBalance)) == false || pounds >= MAX_MONEY_HOLDING) {
		...
		if (pounds < 0 || pence < 0) {
			...
			// Make a unique pointer of new account, and push onto opened accounts vector
...
void AccountManager::viewAllAccounts() const {
	...
	else { // Display all accounts details
...
void AccountManager::viewAccountsIndex(const std::string & index) {
	if (opened_accounts_.empty()) { // Check if any account have been opened
		...
		try {
			...
			// Print out account details for specified index
			std::cout << *opened_accounts_.at(intIndex);
			opened_accounts_.at(intIndex)->printHistory();
...
void AccountManager::transfer(const std::string & takeMoneyAcc, const std::string & giveMoneyAcc, std::string amount) const {
	...
	try {
		...
		// Check given indexes for both receiving and giving accounts has a valid account in them
...
void AccountManager::project(const std::string & years) const {
	...
	if (opened_accounts_.empty()) {
		...
		try {
			...
			std::ios fmt(nullptr);   // Creates a variable of type std::ios to store the stream's format state
			fmt.copyfmt(std::cout);   // Copies the stream's format state into the variable

			// Call compute interest function to find projected value on the account and round output too 2 decimal places
			std::cout << ">- Projected Balance: \x9C" << std::setprecision(2) << std::fixed << chosenSavingsAcc.computeInterest(intYears) << std::endl;

			std::cout.copyfmt(fmt);   // Reset cout stream format state
		}
		catch (const std::exception&) {
...
void AccountManager::search(std::string value) const {
	...
	if (canComputeMoneyInput(pounds, pence, std::move(value)) == false) {  // Check money values can be found
		...
		// Call search function on most recently viewed account
